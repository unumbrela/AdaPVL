...
def load_dinov3_siglip_to_device(cfg):
    ...
    return vision_model.to(cfg.MODEL.DEVICE).eval(), siglip.text_model.to(cfg.MODEL.DEVICE).eval()
...
class CustomCLIP(nn.Module):
    ...
    def encode_text_image(self, tokenized, text_embeds, image):
        ...
        for i, (v_block, t_layer) in enumerate(zip(self.vision_model.blocks, self.text_model.encoder.layers)):
...
def build_medclipseg_adapvl_dinov3_siglip(cfg):
    print("Loading DINOv3 + SigLIP text with AdaPVL")
    vision_model, text_model = load_dinov3_siglip_to_device(cfg)
    vision_model.float()
    text_model.float()

    print("Building AdaPVL + DINOv3+SigLIP")
    model = CustomCLIP(cfg, vision_model, text_model)

    print("Turning off gradients in both the image and the text encoder")
    for name, param in model.named_parameters():
        if any(k in name for k in ["pvl_adapters", "mask_head", "upscale", "vision_proj", "logit_scale", "mlfa", "shared_gate_"]):
