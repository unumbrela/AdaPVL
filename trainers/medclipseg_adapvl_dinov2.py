...
def load_dinov2_to_device(cfg):
    timm_overlay = resolve_timm_pretrained_overlay("vit_base_patch14_dinov2.lvd142m")
    ...
    text_model = AutoModel.from_pretrained(biomedbert_dir, local_files_only=True)
...
class CustomCLIP(nn.Module):
    ...
    def encode_text_image(self, tokenized_prompts, text_prompts, image, attention_mask=None):
        x_img = self.vision_model.patch_embed(image)
        x_img = self.vision_model._pos_embed(x_img)
        ...
        for i, (v_block, t_layer) in enumerate(zip(self.vision_model.blocks, self.text_model.encoder.layer)):
...
def build_medclipseg_adapvl_dinov2(cfg):
    print("Loading DINOv2 + BiomedBERT with AdaPVL")
    vision_model, text_model = load_dinov2_to_device(cfg)
    vision_model.float()
    text_model.float()

    print("Building AdaPVL + DINOv2")
    model = CustomCLIP(cfg, vision_model, text_model)

    print("Turning off gradients in both the image and the text encoder")
    for name, param in model.named_parameters():
        if any(k in name for k in ["pvl_adapters", "mask_head", "upscale", "text_proj", "mlfa", "shared_gate_"]):
