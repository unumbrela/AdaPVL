...
class CustomCLIP(nn.Module):
    def __init__(self, cfg, vision_model, text_model):
        ...
        self.cfg = cfg
        self.vision_model = vision_model  # timm VisionTransformer
    ...
    def encode_text_image(self, tokenized_prompts, text_prompts, image,
                          attention_mask=None):
        # === Vision: timm VisionTransformer (NLD, 1 CLS prefix) ===
...
def build_medclipseg_dinov2(cfg):
    print("Loading DINOv2 + BiomedBERT")
    vision_model, text_model = load_dinov2_to_device(cfg)
    vision_model.float()
    text_model.float()

    print("Building custom DINOv2")
