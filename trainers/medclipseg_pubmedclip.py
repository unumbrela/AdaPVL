...
def load_pubmedclip_to_cpu(cfg):
    ...
    checkpoint_path = download_checkpoint("PubMedCLIP_ViT32.pth")
...
class CustomCLIP(nn.Module):
    def __init__(self, cfg, clip_model, output_hidden_states=False):
        ...
        if cfg.MODEL.BACKBONE == "ViT-B/32":
            self.embed_dim = 768
...
def build_medclipseg_pubmedclip(cfg):

    print(f"Loading CLIP (backbone: {cfg.MODEL.BACKBONE})")
    clip_model = load_pubmedclip_to_cpu(cfg)
