...
class ResidualAttentionBlock(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, text_layer=False):
        super().__init__()
    ...
    def forward(self, inputs):
        ...
        if(not self.text_layer):
            ...
            x_patch = F.linear(v_in, self.attn.out_proj.weight, self.attn.out_proj.bias)
            x = x + self.attention(self.ln_1(x))
            x = x + self.mlp(self.ln_2(x))
            patch_logits.append(x_patch)
            return [x, patch_logits]  # return again as a list, so that nn.seq can work
        else:
            x = x + self.attention(self.ln_1(x))
            x = x + self.mlp(self.ln_2(x))
            return [x]

class ResidualAttentionBlock_CMM(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, text_layer=False):
        super().__init__()
...
class ResidualAttentionBlock_MaPLe(nn.Module):
    ...
    def forward_v(self, inputs: torch.Tensor):
        """
        Forward function for computing the value features for dense prediction (i.e., features for every image patch).
        """
        # Get the weights and biases for the value projection, multihead attention uses 3 * embed_dim for the input projection
    ...
    def forward(self, inputs):
        ...
        if(not self.text_layer):
            ...
            x = x + self.mlp(self.ln_2(x))
            patch_logits.append(x_patch)
            return [x, compound_prompts_deeper, counter, patch_logits]  # return again as a list, so that nn.seq can work
        else:
            x = x + self.attention(self.ln_1(x))
            x = x + self.mlp(self.ln_2(x))
...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, prompts_needed=0,
        ...
        if current_trainer == 'IVLP' or current_trainer == 'VPT':
        ...
        elif current_trainer == 'MedCLIPSeg':
            self.resblocks = nn.Sequential(*[ResidualAttentionBlock_CMM(width, heads, attn_mask,
                                                                              text_layer,)
                                             for i in range(layers)])
            ...
            self.resblocks = nn.Sequential(*[ResidualAttentionBlock(width, heads, attn_mask, text_layer) for _ in range(layers)])
...
class VisionTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int,
        ...
        self.vision_prompt_nctx = design_details["vision_ctx"]
        self.layers = layers
    ...
    def forward(self, x: torch.Tensor, patch_output: bool = False):
        ...
        if patch_output:
            ...
            x, x_all = self.transformer([x, []])
            x_all = torch.stack(x_all, dim=0) # (Num Layers, L, N, D)
            x_patch =  x_all[:, 1:x_all.shape[1] - self.vision_prompt_nctx, :, :] # Remove visual ctx and class token
            x_patch = x_patch.permute(0, 2, 1, 3)  # LND -> NLD

            x_cls =  x_all[:, 0, :, :] # class token
            
            x_patch = self.ln_post(x_patch)
            ...
            if self.proj is not None:
                ...
                # x_patch = x_patch[:,:,1:,:] # remove the class token
                x_patch = x_patch.reshape(self.layers, B, H, W, -1).permute(0, 1, 4, 2, 3).contiguous()
...
class VisionTransformer_MaPLe(nn.Module):
    ...
    def forward(self, x: torch.Tensor, shared_ctx, compound_deeper_prompts, patch_output: bool = False):
        B, C, H, W = x.shape
        ...
        if patch_output:
            ...
            x, _, _, x_patch = self.transformer([x, compound_deeper_prompts, 0, []])
            x_patch = torch.stack(x_patch, dim=0) # (Num Layers, L, N, D)
            x_patch =  x_patch[:, 1:x_patch.shape[1] - self.compound_prompt_nctx, :, :] # Remove visual ctx and class token
            x_patch = x_patch.permute(0, 2, 1, 3)  # LND -> NLD

            # x,  = penultimate([x, compound_deeper_prompts, 0])
            # x = last_resblock.forward_v(x)
            ...
            if self.proj is not None:
                ...
                # x_patch = x_patch[:,:,1:,:] # remove the class token
                x_patch = x_patch.reshape(self.layers, B, hw_shape[0], hw_shape[1], -1).permute(0, 1, 4, 2, 3).contiguous()
            return x_patch
